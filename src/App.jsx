...
function App() {
  ...
  useEffect(() => {
    if (loading) return;
    const subtitle = overlayRef.current.querySelector('.hero-subtitle');
  ...
  useEffect(() => {
    ...
    if (menuOpen) {
      ...
      gsap.to(b2, { rotate: 0, y: 0, duration: 0.3, ease: 'power2.inOut' });
    }
  }, [menuOpen]);
      ...
      <div className="hero">
        ...
        <nav className="hero-nav" style={{ pointerEvents: 'auto' }}>
          ...
          <button
            ...
            onClick={toggleMenu}
          >
            <span ref={bar1Ref} className="menu-bar" />
            <span ref={bar2Ref} className="menu-bar" />
          </button>
